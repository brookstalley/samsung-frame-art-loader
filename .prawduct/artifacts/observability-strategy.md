...
> | `acquisition.deployment_fault` | acquisition refused before it started for a reason no source is at fault for — a full disk, a missing binary, an unwired provider. **At ERROR, and for these three conditions it is the only journal signal there is**: unlike a failed fetch, which is recorded against the source and readable afterwards, these reach the caller as a refusal the tool boundary answers without logging — and the person who can fix them is not the one holding the tool result. **Emitted by the MCP binding rather than by the service**, so the line exists only where acquisition is driven over MCP — `retry_acquisition`'s handler holds the three raise-rather-record translations. That is a real limit rather than a detail: `api-contract.md` records that the browser's acquisition routes are absent only because no screen has needed them yet, and the first one added inherits the refusal without the journal line, leaving an operator debugging "nothing acquires" on a full disk with silence — the exact failure this line was added to break. Moving the translations into `AcquisitionService`, so the signal follows the condition rather than the caller, is **issue #86** |
>
