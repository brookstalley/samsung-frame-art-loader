...
| Add disk headroom | Prune `tile-cache/` and `temp/` — working space, not steady-state storage, sized by the largest single work in flight. **Also `previews/`**, which is different in kind: it holds one cached thumbnail per candidate image instance and **nothing reclaims it**. The sweep that should is specified but unbuilt, and it would reclaim nothing today anyway — it keys on a candidate reaching a terminal verdict, and no shipped surface can set one. Growth follows discovery runs the operator starts rather than a timer, and a preview is disposable by design, so deleting the directory is always safe. It matters here because § Risks opens with the SD card as the top operational risk |
