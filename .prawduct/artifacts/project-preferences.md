...
| **The two planes agree on the heartbeat's name and interval by construction, not by memory** | Test | `tests/preferences/test_heartbeat_contract.py` | advisory | Added 2026-08-07 with the writer. The heartbeat is the only channel from display back to curation, and the plane-isolation norm above forbids the obvious fix — display cannot import curation's reader to share a constant, and curation cannot import display's writer. So the filename and the staleness bound are declared twice in two projects that may not refer to each other, which is a duplicated literal with a norm *requiring* it to stay duplicated. The guard reads both planes' source with AST and compares the values, which is the only mechanism available: it is a claim about two files that cannot see each other, so a linter cannot state it and a runtime test would need both planes running. The failure it catches is silent in the worst way — a renamed file makes curation report "the display plane has not reported" for a plane that is running perfectly and writing its heartbeat to a name nobody reads. |
