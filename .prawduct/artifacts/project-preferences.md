...
| **The stylesheet is the source of truth for token values; this artifact is the source of truth for the rules about them** — norm lives in `design-direction.md` § Direction | Test | `curation/tests/unit/test_design_tokens.py` | strict | Added 2026-08-10 with the design cycle; **ratified by the owner 2026-08-11**, after a round in which it was carried as `proposed` precisely because they had approved the design and not been asked about the norm. The mechanism is real and predates the norm: the test reads the served stylesheet, computes every text and control pair in both schemes, and **refuses any colour written outside the token blocks**. It has narrowed itself once — two non-greedy regexes ate three quarters of the file and a planted `#ff0000` reported clean — so it now asserts its own scan scope, and that assertion is the part to keep. **The gap this row must not hide:** the revised palettes live only in `prototypes/curation-ia-prototype.html`, which the test does not read, so every revised colour is currently hand-checked and ungoverned. It becomes a true `strict` Test row for those values when they land in `app.css`, and not before. |
