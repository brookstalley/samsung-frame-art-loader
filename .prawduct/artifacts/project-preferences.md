...
- **Linting**: ruff, configured per plane. All three select `E,F,I,UP,B,T20,TRY`
  ...
  - **Display** (`display/pyproject.toml`) is the same set as Curation — adds
    ...
    `target-version = py312`, which is the **declared floor** the tools are held
    to rather than a preference. It is *not* the Pi's interpreter: Raspberry Pi OS
    Trixie ships 3.13 and the ratified decision in
    `platform-and-dependency-findings.md` targets 3.13, with 3.12 kept as a floor
    that nothing expects to land on. Targeting the floor is the deliberate choice —
    formatting that only parses on 3.13 would break the range the manifest says it
    supports. _(Corrected 2026-08-05: this row read "which is the Pi's interpreter",
    inverting the ratified decision in the norm index a Norm Health sweep reads.)_
    It arrived with the plane's manifest and **before any module**, so
...
|---|---|---|---|---|
| **The television belongs to whoever is using it** — norm lives in `nonfunctional-requirements.md` § The television belongs to whoever is using it | Test | `display/tests/` — the art-mode gate is guarded today; the power gate's guard lands with Chunk 27 and is named in its acceptance criteria | janitor | **Added 2026-08-17, with the amendment that gave the plane a power key — and the row is late.** The norm has bound this product since 2026-08-07 and was never indexed, so the index has been reporting a smaller registry than the one in force; found while amending it, and fixed here rather than filed. Mechanism is Test rather than Critic because that is what the norm's own history argues for: the interruption it exists to prevent shipped through a deliberate decision, a review, and a green suite, and what caught it was the operator losing the picture they were watching. A rule enforced only by reading is a rule enforced by whoever read most carefully that day. **Its two verbs need two guards** — the selection gate reads `get_artmode`, the power gate needs `PowerState` as well, because dark and television are read-identical on the art channel and only one of them may be pressed at. |
...
| black formatting, line-length 130 | Linter (the formatting) + Test (the plane boundary) | `[tool.black]`, every plane's `pyproject.toml`; `tests/preferences/test_tool_config.py` | janitor | Already configured; removes formatting from review entirely. *(Corrected 2026-08-01: the artifact read `pyproject.toml` singular, written when the repo had one. Every plane carries a `[tool.black]` at line-length 130 and its own `target-version`, and a reader checking the norm against "the" pyproject would have audited part of it.)* *(Corrected 2026-08-05: this row said "both `pyproject.toml`s" and named the two targets, written when there were two planes; `display/` made it three the day the ruff row directly above was corrected for the same reason and this one was not. Stated as "every plane" rather than as a new count, because the count is what went stale twice — and `tests/preferences/test_tool_config.py` guards the root's plane exclusions, not the per-plane targets, so nothing mechanical catches the next one either.)* *(Extended 2026-08-02 by the norm sweep: the root config drew the plane boundary for ruff and not for black, so `black .` at the root walked both planes — 103 files, not 18 — and formatted curation under `py312`. Fixed, and given a Test half, because the boundary is now drawn twice in two syntaxes (ruff takes a list of paths, black a regex) with nothing making them agree. The output was byte-identical either way, which is the whole argument for a mechanical guard: the split had stopped holding for formatting while still holding for lint, and no green suite anywhere would have said so.)* |
...
| No hardcoded deployment values in source (IP, art root, coordinates) | Test | `tests/test_config.py::test_no_source_file_carries_a_deployment_value` | advisory | The same code runs on the Pi and on a dev Mac; a hardcoded `/srv/art` means the dev path is a source edit, which is how config drift starts. **The test carries both generations of the path** — the live one and the retired `/home/tvpi`, which the recovered 2024 unit still names in every line — because retiring the dead path without adding the live one leaves a guard that is green for the wrong reason. |
...
| **WCAG 2.1 AA on the curation UI, and colour is never the sole carrier of state** — decision lives in `project-state.yaml` § `design_decisions.accessibility_approach`, norm lives in `accessibility-spec.md` § Direction and its requirements are stated in that artifact's § The curation browser *(added 2026-08-11 — the decision had no artifact until then, so this row pointed at a YAML field for a rule a builder needs prose for)* | Test | `curation/tests/unit/test_design_tokens.py` | advisory | Added 2026-08-01 with the first browser surface. The contrast half is fully mechanical: the test reads the real token values out of the served stylesheet, computes every text and control pair in both colour schemes, and refuses any colour written outside the token blocks — so "AA verified" cannot rot into a sentence with nothing under it. The non-colour half stays judgment-required: a test can see that a badge has a glyph, not that the glyph distinguishes anything. |
| **The e-paper label is legible at standing distance: the panel is driven at 16 greys, and type never shrinks below the floor — content drops instead, except the facts that identify the work, which shrink rather than vanish** — norm lives in `accessibility-spec.md` § Direction, requirements in § The e-paper label | Test | `display/tests/test_epaper.py` (the mode is set and a panel that quietly stays in one bit is refused) and `display/tests/test_label_layout.py` (the two tiers, the name ladder and the drop rule, against an injected measurer), `display/tests/test_label_properties.py` (both tiers as invariants over every content shape) and `display/tests/test_label_and_heartbeat_wiring.py` (the shrink reaches the journal) | advisory | **RATIFIED 2026-08-11 by the operator, and amended by the same ruling — the flat clause became two tiers.** Mandatory content (the artist's family name, the artist's given name, and the title when there is one) shrinks as far as it must rather than dropping; everything else drops at full size rather than shrinking, and **nothing else is mandatory**. The tiers, and the condition the amendment rests on — a shrink is journalled exactly like a drop — are in `accessibility-spec.md` § Type never shrinks to fit. **Both halves are built and both are guarded, 2026-08-13.** The mandatory tier shrinks rather than dropping and journals every shrink as `label.shrunk` at WARNING — the condition the amendment rests on, and the reason the Test column names the wiring suite as well as the layout one. **`display/tests/test_label_properties.py` is the artifact that carries the norm's universal form**: ten of a work's fields are independently present or absent, so the two defects this closed were both about *which* fields a record happened to have rather than about any one line, and an example suite is structurally unable to state "no optional fact is ever set below the floor". Naming it here is the point of this column — an enforcement artifact no row points at is one a refactor deletes with nothing noticing, which this table's own cautionary row exists to record. Added with `accessibility-spec.md`. The builder's reading is that this is an under-claiming *correction* rather than a norm birth, and most of it is traceable: the 16-grey requirement and the mode read-back are in `nonfunctional-requirements.md` § Output Quality, and standing-distance legibility is in `design_decisions.accessibility_approach`. **But the drop-rather-than-shrink clause appears in no artifact before this row**, and its only prior record is the `layout.py` docstring written by the very chunk whose tests this row names as enforcement — which is the laundering shape, whatever the intent. That sentence from the owner arrived — see the RATIFIED note this row opens with, which is what closed it. `design_decisions.accessibility_approach` has said since discovery that this product has two accessibility surfaces and **the important one is physical** — read by household members and guests at standing distance, on an unbacklit panel, with no interface to navigate. The row above it named only the browser, so the norm that outranks it had no row and its two real mechanisms had nothing pointing at them: an enforcement artifact no row names is one a refactor can delete, rename or narrow with nothing noticing. Both guards landed with Chunk 13A. The greyscale half is the one that would otherwise be invisible — `max_colors` reports 16 in **both** modes, so the sanity check anyone would write cannot tell 1-bit from 16-level, and the 2024 plane rendered 1-bit type for years with nothing reporting it. **Test rather than Critic for both, because both are mechanically checkable by construction**; what is *not* covered and must not be read as covered is the type sizing itself, which is `open` in the spec and closable only by the operator at the panel. |
...
| **The two planes agree on the heartbeat's filename and its instant's key by construction, not by memory** | Test | `tests/preferences/test_heartbeat_contract.py` | advisory | Added 2026-08-07 with the writer. The heartbeat is the only channel from display back to curation, and the plane-isolation norm above forbids the obvious fix — display cannot import curation's reader to share a constant, and curation cannot import display's writer. So `HEARTBEAT_FILENAME_TEMPLATE`, `REPORTED_AT_KEY` and — since 2026-08-12 — `MANIFEST_FILENAME_TEMPLATE` are declared twice in two projects that may not refer to each other, which is a duplicated literal with a norm *requiring* it to stay duplicated. *(Corrected 2026-08-12: this row named `HEARTBEAT_FILENAME`, a constant that stopped existing when the heartbeat went per wall and became a template, and it named two constants where the guard now compares three. Under-claiming in the direction this table's own rule warns about — a reader would have believed the manifest's filename was unguarded, and it is the more consequential of the two: a display that resolves the wrong manifest name serves the wrong room's pictures.)* The guard reads both planes' source with AST and compares the two values, then pins them literally against `observability-strategy.md` so that both sides moving together is still a break; it is the only mechanism available, since a claim about two files that cannot see each other is not something a linter can state and a runtime test would need both planes running. The failure it catches is silent in the worst way — a renamed file, or an instant spelled `timestamp`, makes curation report "the display plane has not reported" for a plane that is running perfectly. *(Corrected 2026-08-10: this row was titled "name **and interval**" and said "the filename and the staleness bound are declared twice". Neither is true — the guard compares two string constants, curation declares no interval or staleness bound at all, and `observability-strategy.md` § The Health Surface says outright that nothing downstream compares the age to a threshold. That is the over-claiming shape this table records a rule about, three rows down, in a row written by the same hand a day earlier: **an index that over-claims sends a reader away believing a divergence would be caught.** The guard is real and does exactly what it now says.)* |
| **The stylesheet is the source of truth for token values; this artifact is the source of truth for the rules about them** — norm lives in `design-direction.md` § Direction | Test | `curation/tests/unit/test_design_tokens.py` | strict | Added 2026-08-10 with the design cycle; **ratified by the owner 2026-08-11**, after a round in which it was carried as `proposed` precisely because they had approved the design and not been asked about the norm. The mechanism is real and predates the norm: the test reads the served stylesheet, computes every text and control pair in both schemes, and **refuses any colour written outside the token blocks**. It has narrowed itself once — two non-greedy regexes ate three quarters of the file and a planted `#ff0000` reported clean — so it now asserts its own scan scope, and that assertion is the part to keep. **The gap this row recorded is closed, 2026-08-12:** the revised light and dark palettes now live in `app.css`, so the test reads them and this is a true `strict` Test row for those values — which it was not while they sat in `prototypes/curation-ia-prototype.html`, hand-checked and ungoverned. **The residue is half closed, 2026-08-12.** It read: `--good`/`--warn`/`--crit`, their `-quiet` variants and `--scrim`/`--scrim-text` landed with no rule referencing them, so the pair computation did not reach them and they were verified by hand — *they come under the mechanism when the masthead health indicator and the scrim consume them, and not before.* The masthead indicator consumed the status tokens, and the pair computation now reaches all three of them and their quiet grounds, in both schemes and against the control-boundary floor as well as the text one. `--crit` is covered though nothing renders in it yet, because the pair is a fact about the palette rather than about the component. **`--scrim`/`--scrim-text` remain outside it**: they are `rgb()` with alpha over an unknown photograph, which is not a pair this computation can express, and the Walls screen is the chunk that first lays type on one. |
| **The curation surface is organised around what a curator does, never around the pipeline's stages** — norm lives in `information-architecture.md` § Direction | Critic | — | janitor | Added 2026-08-10; **ratified by the owner 2026-08-11**. Judgment-required and that is why it cannot be a test: the violation is a destination named for a subsystem rather than for an intention, which has no import signature and no grep. What makes it worth a norm is that the failure is invisible at the only scale anyone reviews at — the built surface's five tabs were the pipeline's stages in pipeline order, and *every one of them was correct as the chunk that produced it*. No per-chunk review could have caught the sum. The next subsystem to acquire a UI will want a sixth tab for the same locally-good reasons. **The retroactivity note is retired, 2026-08-12.** It read: *the built surface does not conform, and the artifact is the plan for making it do so, so this row will read as violated until that work lands.* That work has landed. The five pipeline-stage tabs are three destinations — the Walls, Collection, Discover — Health is a masthead status indicator that expands to a screen it is no longer a peer of, and Review and Theme are contextual screens on real addresses. **The mechanism stays `Critic`, and nothing about landing the work makes it mechanical**: what a test can hold is that the navigation is these three and no more (`curation/tests/browser/test_the_three_destinations.py`), and what it cannot hold is whether a fourth entry names an intention or a subsystem — which is the whole of the violation. The structural half of the same work *is* mechanised and is a different rule: no screen module imports another, recorded in `architecture.md` § Components & Responsibilities and enforced by `curation/tests/unit/test_client_module_boundaries.py`. |

### Deliberately uncovered (ruled, not overlooked)

Mechanical guards this index knowingly does **not** name as norm enforcement. They
are recorded here because the alternative is that every Norm Health sweep
rediscovers them and re-raises the same question — which is what issue #40 was,
and what this section exists to stop happening a second time.

**A guard listed here is still a guard.** Nothing below is deprecated, and
deleting one is a change worth reviewing on its own terms. What the ruling says
is narrower: it is not a *norm this product states*, so it gets no row, and its
absence from the index above is a decision rather than a gap.

| Guard | Ruling (owner, 2026-08-06) | Reasoning |
|---|---|---|
| `tests/test_repo_hygiene.py::test_no_python_source_is_gitignored` and `tests/test_repo_hygiene.py::test_no_package_directory_is_gitignored` | Not a stated norm — repository hygiene | A file excluded from version control is invisible to review, which is a real failure and a generic one: it is true of every repository and carries nothing specific to this product's risk. The guards stay because they are cheap and the failure is silent; what the index holds is the set of rules *this product* has decided, and a rule every project shares is not one of them. Contrast the token row above, which is this product's own incident. |
| `curation/tests/unit/test_persistence_boundary.py::test_only_the_durable_store_imports_the_storage_driver` | Not a stated norm — the layering norm above already covers the ground | The adjacent row, **"Operation logic lives ONLY in the service layer"**, keeps its `Critic` mechanism and does **not** gain this file as an enforcement artifact. The two boundaries are one rung apart: that norm is about where *decisions* live, this guard is about where a *driver import* may appear, and naming an import check as the enforcement of a judgement-required norm would over-claim in the other direction — a green AST check reading as though the layering rule were mechanically enforced, when the violations it exists to catch (a handler that validates, orders or decides) are exactly what an import check cannot see. |
| `tests/preferences/test_norm_index.py` | Not a stated norm — the index's own plumbing | Its positional claim is the strongest of the four: it lives in the directory this table designates as the home of Test-mechanism norms, and it enforces a property of the index. The ruling is that this makes it *infrastructure of* the index rather than a norm *in* it — a row naming it would make the table an entry in itself, and the recursion buys nothing a reader can act on. It also deliberately checks rows→artifacts only; **generalising the reverse direction stays out of scope**, and issue #40's own scope-out is worth carrying forward: two probes have been run against that direction and each one found something, so nobody should read "swept, filed, done" into this section. |

*Recorded 2026-08-06, closing issue #40. The filing was `stage: requirements` and
deliberately not buildable, because authoring a norm is inventing a requirement —
so all four dispositions above are the owner's ruling, not a builder's inference.*
