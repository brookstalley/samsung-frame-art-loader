...
| Catch specific exceptions; broad catch needs `# prawduct:allow prawduct/broad-except -- reason` | Critic | — | advisory | A swallowed exception in an unattended loader shows up as "the TV just stopped changing", with nothing in the log to say why. *(Corrected 2026-08-02: this row claimed Mechanism "Critic + linter" and Enforcement artifact "ruff `B`/`TRY`, both `pyproject.toml`s". **Neither rule set flags `except Exception:`** — bugbear has no blind-except rule and every selected `TRY` rule is about raise or log shape. The rule that would is `BLE001`, and `BLE` is selected nowhere here, so the norm has been Critic-only since it was written while the index said otherwise. The live proof is `tvart.py`'s unwaived swallowing catch inside `upload_file`, on which `ruff check .` is green. Selecting `BLE` is not a one-line fix and is why this is a correction rather than a repair: the sanctioned waiver is a Prawduct pragma, not a ruff directive, so `BLE` would fail the repo's one correctly-waived catch alongside the unwaived one — closing it needs a paired `# noqa: BLE001` convention written into this row. Tracked in the backlog.)* |
