...
## A docstring's safety argument is a claim about the code beside it — pin it, don't just state it
When a comment names a failure mode as unacceptable ("dropping this would merge two works, the direction with no recovery"), the next thing written is the test proving that failure cannot occur — derived from the *docstring*, not from the diff. Stating a danger reads as defending against it and is not: a rule shipped here argued at length that a trailing hostname alone is not evidence because `Composition No.5` looks like one, then required only a bracketed URL beside it, so `Composition No.5 (https://example.com/x)` cleaned to `Composition`. Because a mutation sweep only asks whether the lines you *wrote* are defended, it cannot see a guard you reasoned about and never wrote. Detail: `learnings-detail.md`.

## Two redundant defences look exactly like two undefended branches in a mutation sweep
If one input is rescued by two independent guards, mutating either alone leaves the tests green and both report SURVIVED — which is indistinguishable from real coverage gaps and sends you writing tests for branches that are already safe. When a sweep survivor surprises you on a line you believe is load-bearing, check whether a *sibling* guard is covering the same case before writing anything: the fix is a case per guard that the other cannot rescue, not a broader test. Detail: `learnings-detail.md`.

## A bug report's stated CAUSE is a hypothesis, at the same standard of proof as its symptom
The symptom is usually observed; the cause is usually reasoned, and gets recorded in artifacts as though it were observed too. Before building on a filed cause, run the cheapest experiment that could refute it — here, feeding the model's plausible output back through the cleaner reproduced the "model-emitted" corruption character for character, in one command, and moved the fix from the provider's schema to our own regex. A cause that was never executed is a guess with a citation. Detail: `learnings-detail.md`.
