...
## A mutation sweep that flips verdict between identical runs is a property test finding its case by luck, not flaky tooling — when a sweep's result changes with no source change, apply the mutation by hand and search the input space for an observable difference; if the difference is real and rare, the fix is an example test naming the case, because a property that reaches it by chance defends nothing
