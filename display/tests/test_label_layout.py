...
def as_lines(*texts: str) -> tuple[Line, ...]:
    """These strings as unstyled lines.
    ...
    **Unstyled on purpose.** Every decision this module makes — the ordering, the
    ...
    """
    return tuple((Run(text),) for text in texts)
...
class TestTheHierarchy:
    ...
    def test_the_leading_line_gets_the_primary_tier(self):
        layout = lay_out(as_lines("Chicago", "Georgia O'Keeffe", "American"), PANEL, measured, SCALE)

        assert layout.blocks[0].size_px == SCALE.primary_px

    def test_everything_after_the_leading_line_sits_at_the_floor(self):
        layout = lay_out(as_lines("T", "A", "N", "D", "Y", "M", "X"), PANEL, measured, SCALE)
    ...
    def test_nothing_is_ever_set_between_the_two_tiers(self):
        ...
        **Asserted on the sizes the layout actually emits, not on `TypeScale`'s
        ...
        exists to catch.
        """
        lines = as_lines("The Banquet", "Jan Steen", "Dutch", "1626–1679", "Oil on canvas", "25 × 37 cm")
    ...
    def test_nothing_is_ever_set_below_the_floor(self):
        ...
        for surface in (PANEL, Geometry(width_px=400, height_px=300, margin_px=10)):
            layout = lay_out(as_lines("The Banquet", "Jan Steen", "Dutch", "Oil on canvas"), surface, measured, SCALE)
    ...
    def test_a_device_read_from_further_away_sets_larger_type(self):
        ...
        near_layout = lay_out(as_lines("Chicago", "Georgia O'Keeffe"), PANEL, measured, SCALE)
        far_layout = lay_out(as_lines("Chicago", "Georgia O'Keeffe"), PANEL, measured, far)

        assert far_layout.blocks[0].size_px > near_layout.blocks[0].size_px

    def test_the_text_is_carried_through_unchanged(self):
        layout = lay_out(as_lines("Cow's Skull with Calico Roses", "Georgia O'Keeffe"), PANEL, measured, SCALE)
...
class TestPlacement:
    def test_it_starts_at_the_top_left_margin(self):
        layout = lay_out(as_lines("Silver Sun"), PANEL, measured, SCALE)
    ...
    def test_every_block_sits_at_the_left_margin(self):
        layout = lay_out(as_lines("The Banquet", "Jan Steen", "Dutch"), PANEL, measured, SCALE)
    ...
    def test_the_gap_between_blocks_is_proportional_to_the_upper_one(self):
        layout = lay_out(as_lines("The Banquet", "Jan Steen"), PANEL, measured, SCALE)
    ...
    def test_a_wrapped_line_is_given_the_room_it_actually_takes(self):
        ...
        layout = lay_out(as_lines(long_title, "Frank Lloyd Wright"), PANEL, measured, SCALE)
...
class TestWhatComesOffWhenItWillNotFit:
    ...
    def tall_enough_for(count: int, *, margin_px: int = 20) -> Geometry:
        ...
        **Taken from where the blocks actually land, not recomputed here.** These
        ...
        last = lay_out(as_lines(*TestWhatComesOffWhenItWillNotFit.LINES), unbounded, measured, SCALE).blocks[count - 1]
    ...
    def test_the_least_identifying_lines_come_off_the_bottom(self, count: int):
        ...
        """
        layout = lay_out(as_lines(*self.LINES), self.tall_enough_for(count), measured, SCALE)
    ...
    def test_nothing_placed_runs_past_the_bottom_margin(self):
        ...
        layout = lay_out(as_lines("The Banquet", "Jan Steen", "Dutch", "1626–1679", "Oil on canvas"), short, measured, SCALE)
    ...
    def test_the_title_is_kept_even_when_it_alone_overflows(self):
        ...
        layout = lay_out(as_lines("Triptych Window from the Coonley Playhouse", "Frank Lloyd Wright"), tiny, measured, SCALE)
...
class TestDegenerateSurfaces:
    ...
    def test_there_is_nowhere_to_put_anything_and_it_says_so(self, surface: Geometry, why: str):
        layout = lay_out(as_lines("The Banquet", "Jan Steen"), surface, measured, SCALE)
...
class TestTheMeasure:
    ...
    def test_a_line_wraps_at_the_measure_rather_than_at_the_surface_edge(self):
        ...
        lay_out(as_lines("Chicago", "Georgia O'Keeffe", "American"), self.UNBOUNDED, self.recording(seen), SCALE)
    ...
    def test_a_surface_narrower_than_the_measure_still_governs(self):
        ...
        seen: list[tuple[str, int, int]] = []
        lay_out(as_lines("The Bedroom", "Vincent van Gogh"), narrow, self.recording(seen), SCALE)
    ...
    def test_the_measurer_is_given_the_runs_and_not_their_text(self):
        ...
        lay_out(((Run("Hokusai", case=Case.CAPITALS),),), PANEL, measure, SCALE)

        assert seen == [(Run("Hokusai", case=Case.CAPITALS),)]
    ...
    def test_a_block_carries_the_width_it_was_measured_at(self):
        ...
        seen: list[tuple[str, int, int]] = []
        layout = lay_out(as_lines("Chicago", "Georgia O'Keeffe", "American"), PANEL, self.recording(seen), SCALE)
    ...
    def test_the_bound_is_what_makes_a_long_line_wrap(self):
        ...
        layout = lay_out(as_lines("T", "A", long_line), self.UNBOUNDED, measured, SCALE)
...
class TestGeometryIsAParameter:
    def test_two_surfaces_of_different_size_get_different_layouts(self):
        ...
        narrow = Geometry(width_px=400, height_px=1072, margin_px=40)
        lines = as_lines("Triptych Window from the Coonley Playhouse, Riverside, Illinois", "Frank Lloyd Wright")
