...
def compose(
    ...
    **The artwork is not centred on the canvas — it is centred in the artwork
    ...
    # **Only the decode is translated, and the boundary matters.** Reading the
    # source can fail on bytes a museum served; writing the canvas can fail
    # because this host's disk is full or `ready/` has the wrong owner. Those send
    # whoever reads the message to opposite places, so wrapping the whole function
...
def _fit_into_box(image: Image.Image, panel_width: int, panel_height: int, box: ArtworkBox) -> tuple[Image.Image, FitAssessment]:
    ...
    the write that follows it to raise on its own terms.
    """
