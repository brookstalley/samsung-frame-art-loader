...
def test_a_run_started_from_discover_has_no_committing_turn(talking, runner, discovery):
    ...
    A committed thread is seeded beside the Discover run because the claim is a
    *discrimination* rather than an absence: over a store where nothing was ever
    committed, "no turn carries this run" holds however completely the seam is
    broken, and the assertion would range over nothing. With two runs alive at
    once, what is asserted is that exactly the conversation's is written onto a
    turn — and onto the one turn that committed it.
    """
    from curation.persistence.discovery_records import InitiatedBy

    conversation_id = talking.start().conversation.id
    talking.speak(conversation_id, "Something calm.")
    seeded = talking.commit(conversation_id, "Agnes Martin").turns[-1].turn.committed_run_id
    ...
    turns = [view.turn for conversation in talking.list_conversations() for view in talking.get(conversation.id).turns]
    assert [turn.committed_run_id for turn in turns] == [None, None, seeded]
    assert all(turn.committed_run_id != run.id for turn in turns)
