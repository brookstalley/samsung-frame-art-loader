...
class TestTheFacetValueIsCaseInsensitiveInEveryReadThatTouchesIt:
    """ "Baroque" and "baroque" are one value, decided by the column.

    Every read of a facet value has to agree about this — the uniqueness that
    the counts rest on, the filter's match, the `GROUP BY` the counts are taken
    over, and the vocabulary's ordering. Declared on the column they cannot
    ...
    def test_the_same_value_in_another_case_is_the_same_facet(self, service: CatalogueService):
        ...
        # A no-op returning the row already held, not a refusal — the same answer
        # the matching-case call gets. The service's own check had to be taught
        # the column's collation for that to be true: an exact comparison sent
        # the write down to an index that refused it, so the documented no-op
        # became an error for the same claim in different capitals.
        again = service.record_facet(
            artwork_id=work.id, kind=VocabularyKind.MOVEMENT, value="baroque", derivation=FacetDerivation.INFERRED
        )

        assert again.value == "Baroque", "the first recording of a claim is the spelling that survives"
        assert [facet.value for facet in service.facets_for(work.id)] == ["Baroque"]

    def test_a_filter_typed_in_another_case_still_finds_the_work(self, faceted: CatalogueService):
        listing = faceted.list_artworks(facets={VocabularyKind.MOVEMENT: ["BAROQUE"]})

        assert sorted(entry.artwork.title for entry in listing.entries) == [
            "Interior with Four Windows",
            "The Weight of Salt",
        ]

    def test_one_rail_rather_than_two_when_the_catalogue_holds_both_spellings(self, faceted: CatalogueService):
        # Written through the store so the service's own guards cannot be what
        # makes this pass: the claim is about the file's collation.
        work = faceted.add_artwork(title="A Room Facing North")
        faceted.record_facet(
            artwork_id=work.id, kind=VocabularyKind.MOVEMENT, value="baroque", derivation=FacetDerivation.INFERRED
        )

        movements = next(group for group in faceted.list_artworks().facets if group.kind is VocabularyKind.MOVEMENT)
        baroque = [option for option in movements.options if option.value.lower() == "baroque"]

        assert len(baroque) == 1, "one value in two cases must offer one control, not two half-counted ones"
        assert baroque[0].count == 3


class TestAFileFromALaterBuildDoesNotBreakTheOneReadingIt:
    """A facet kind this build does not know is skipped, in every read alike.

    The downgrade case: a later version records `palette=warm`, the operator
    rolls back, and the older build meets a row whose `kind` its enum has no
    member for. The collection listing was already careful about this; the two
    single-work reads were not, and they raised `ValueError` — which is not a
    `StorageError`, so it passed the service's translation untouched and reached
    the caller as a 500 rather than as a sentence. One unrecognised row must not
    make a work unreadable when it already cannot make the collection unreadable.
    """

    @staticmethod
    def _write_unknown_kind(store: SqliteDurableStore, artwork_id: str) -> None:
        """A row no enum member matches, written under the store's own guards."""
        store.upsert(
            "work_facets",
            pk=("id",),
            row={
                "id": "facet-from-the-future",
    ...
    def test_the_work_still_reads_and_keeps_the_facets_this_build_knows(self, store, service, catalogue_file):
        work = service.add_artwork(title="Interior with Four Windows")
        service.record_facet(
            artwork_id=work.id, kind=VocabularyKind.MOVEMENT, value="Baroque", derivation=FacetDerivation.INFERRED
        )
        self._write_unknown_kind(catalogue_file, work.id)

        assert [facet.value for facet in service.facets_for(work.id)] == ["Baroque"]

    def test_the_collection_listing_agrees_with_it(self, store, service, catalogue_file):
        """The two reads must not disagree about the same row — that was the defect."""
        work = service.add_artwork(title="Interior with Four Windows")
        self._write_unknown_kind(catalogue_file, work.id)
        ...
        assert listing.total == 1
        assert all(group.kind is not None for group in listing.facets)


class TestThePageAndItsCountsAreReadAtOneInstant:
    """The listing's parts are read under one scope, so they cannot disagree.

    **This is why the counts are allowed to be in this response at all.** The
    contract rejects a second route on the grounds that a count fetched
    separately could describe a catalogue the page never saw; that reasoning is
    only worth anything if the parts here are actually read together. The page,
    its total, the vocabulary, every kind's counts and the artist resolution are
    four statements or more, and handlers are synchronous `def` — Starlette runs
    them in a worker thread, so a write landing between any two is a real window.

    **Asserted structurally, and deliberately so.** The alternative is to race a
    writer against a reader and assert on the outcome, which is a flake at best
    and passes vacuously when the timing happens not to interleave. What is
    actually being claimed is "these reads happen inside one scope", and that is
    a thing the call order states exactly. The same reasoning `test_plane_
    isolation.py` uses when it parses imports rather than trying to observe a
    forbidden call at runtime.
    """
    ...
    def test_every_read_the_listing_composes_falls_inside_one_scope(self, store, service):
        for title, movement, era, subject in _CORPUS:
            work = service.add_artwork(title=title)
            for kind, value in (
                (VocabularyKind.MOVEMENT, movement),
        ...
        recording = self._Recording(store)
        listing = CatalogueService(recording).list_artworks(facets={"movement": ["Baroque"]})
        ...
        assert {"list_artworks", "facet_vocabulary", "count_facet_values"} <= inside
        # And nothing the listing composes is read after the file is released
        # again — a read outside is a part of the answer from a different instant.
        assert not [
            event for event in events[closed + 1 :] if not event.startswith("reading")
        ], f"read outside the scope: {events[closed + 1 :]}"
