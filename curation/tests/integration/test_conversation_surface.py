...
from curation.persistence.discovery_records import InitiatedBy, SpendCategory
...
def test_a_run_started_from_discover_is_on_no_conversation_turn(server_url):
    """The other half of the seam's claim: it is set only where it happened.

    A committed thread is seeded first, because the claim discriminates rather
    than merely denies: over an empty conversation list, "no turn carries this
    run" holds however completely the seam is broken. Two runs are alive here and
    they are alike in provenance — both `web_ui`, since Discover and a commit are
    the same surface — so the transcript is the only thing that tells them apart,
    ...
    seeded = committed["committed_run_id"]
    ...
    assert [entry["conversation_id"] for entry in listed["conversations"]] == [conversation_id]
    turns = [
        turn
        for entry in listed["conversations"]
        for turn in httpx.get(f"{server_url}/api/conversations/{entry['conversation_id']}", timeout=20).json()["turns"]
    ]
    assert [turn["committed_run_id"] for turn in turns] == [None, None, seeded]
    assert all(turn["committed_run_id"] != run["run_id"] for turn in turns)
    # Neither run is distinguishable by which surface asked for it.
    seeded_run = httpx.get(f"{server_url}/api/runs/{seeded}", timeout=20).json()["run"]
    assert run["initiated_by"] == seeded_run["initiated_by"] == InitiatedBy.WEB_UI.value
